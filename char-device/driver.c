...
static void ddone_device_write_mem8(struct ddone_device *dev, u32 offset, u8 val);
...
static ssize_t chardev_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos){
	...
	while(ddev->bytes_to_write != 0){
		...
		mutex_unlock(&ddev->mutex);
		err = wait_event_interruptible(ddev->rq, ddev->bytes_to_write == 0);
		mutex_lock(&ddev->mutex);
		if(err){
...
static int device_probe(struct platform_device *pdev){
	...
	ddev = devm_kzalloc(&pdev->dev,sizeof(struct ddone_device),GFP_KERNEL);
